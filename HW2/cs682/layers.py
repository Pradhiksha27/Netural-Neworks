...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        z = (x - mu) / (std)
        out = (gamma * z) + beta
        cache = {'x':x,'axis':layernorm,'mean':mu,'gamma':gamma,'std':std,'var':var,'z':z}
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    a = dfdz * dzdx
    b = np.sum(dfdz * -dzdx, axis = 0)/N
    c = np.sum(dfdz * dzdv, axis = 0) * (dvdx + (dvdu/N))
    dx = a + b + c
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    assert (H - H1 + 2 * pad) % stride == 0 
    assert (W - W1 + 2 * pad) % stride == 0 
    
    H_out = 1 + (H - H1 + 2 * pad) // stride
    W_out = 1 + (W - W1 + 2 * pad) // stride
    ...
    w_row = w.reshape(F, C*H1*W1)                        
    x_col = np.zeros((C*H1*W1, H_out*W_out)) 
    
    for index in range(N):
        n = 0 
        for i in range(0, padded_H-H1+1, stride): 
            for j in range(0, padded_W-W1+1,stride): 
                x_col[:,n] = padded_x[index,:,i:i+H1,j:j+W1].reshape(C*H1*W1) 
                n += 1
        out[index] = (w_row.dot(x_col) + b.reshape(F,1)).reshape(F, H_out, W_out)
...
def conv_backward_naive(dout, cache):
    ...
    ###########################################################################  
    padded_x, w, b, conv_param = cache
    stride = conv_param['stride']
    pad = conv_param['pad']
    N, F, H_out, W_out = dout.shape
    N, C, padded_H, padded_W = padded_x.shape
    H1, W1 = w.shape[2], w.shape[3]

    dx = np.zeros((N, C, padded_H - 2*pad, padded_W - 2*pad))
    ...
    for index in range(N):
        ...
        out_col = dout[index].reshape(F, H_out *W_out)          
        w_out = w_row.T.dot(out_col)                         
        dx_cur = np.zeros((C, padded_H, padded_W))
        n = 0
   
        for i in range(0, padded_H-H1+1, stride):
            for j in range(0, padded_W-W1+1, stride):
                
                dx_cur[:,i:i+H1,j:j+W1] += w_out[:,n].reshape(C,H1,W1)
                x_col[:,n] = padded_x[index,:,i:i+H1,j:j+W1].reshape(C*H1*W1)
                n += 1
                
        dx[index] = dx_cur[:,pad:-pad, pad:-pad]
        dw += out_col.dot(x_col.T).reshape(F,C,H1,W1)
    ...
    for index in range(N):
        ...
        for i in range(0, H - PH + 1, stride):
            for j in range(0, W - PW + 1, stride):
                ...
                out_col[:,n] = pool_region.max(axis=1)
                n += 1
        out[index] = out_col.reshape(C, H_out, W_out)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for index in range(N):
        ...
        dout_row = dout[index].reshape(C, H_out*W_out)
        n = 0
        ...
        for i in range(0, H-PH+1, stride):
            for j in range(0, W-PW+1, stride):
                ...
                dout_curr = dout_row[:,n]
                n += 1
                
                dmax_pool = np.zeros(pool_region.shape)
                dmax_pool[np.arange(C),max_pool_indices] = dout_curr
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    cache = {'gamma':gamma,'std':std,'size':size,'z':z}
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def spatial_groupnorm_backward(dout, cache):
    ...
    M = z.shape[0]
    dfdz = (dout * cache['gamma']).reshape(size).T
